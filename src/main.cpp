...
int main() {
    ...
    for (int opt = 0; opt < 2; opt ++)
    {
        ...
        for (int j = 0; j < numIdentities; j++)
        {
            ...
            {
                Eigen::Vector3f tens_vec = shapeTensor(j, 22, i); //23
                cv::Point3f conv_vec;
        ...
        for (int i = 0; i < numExpressions; i++)
        {
            ...
            for (int j = 0; j < n_vectors; j++) {
                Eigen::Vector3f tens_vec = shapeTensor(137, i, j); //138
                cv::Point3f cv_vec;
                cv_vec.x = tens_vec.x();
                cv_vec.y = tens_vec.y();
                cv_vec.z = tens_vec.z();
        ...
        // optimize identity
        identityOptimize(multIdn, lmsVec, poseVec, image, f, identity_w);
    ...
    /*
    //faceVerts Alternate
    vector<easy3d::vec3> faceVertsAlt;
    faceVertsAlt.reserve(numDenseVerts);
    ...
    for (int i = 0; i < numDenseVerts; i++)
    {
        ...
        faceVertsAlt.push_back(easy3d::vec3(x, y, z));
    }

    vector<easy3d::vec3> faceVertsAlt2;
    faceVertsAlt2.reserve(numDenseVerts);
    ...
    for (int i = 0; i < numDenseVerts; i++)
    {
        ...
        faceVertsAlt2.push_back(easy3d::vec3(x, y, z));
    }
    */
    ...
    //change from vector to matrix format
